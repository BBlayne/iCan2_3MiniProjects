...
public class UnityChanSuperController : SuperStateMachine
{
    ...
    void Walk_SuperUpdate()
    {
        ...
        if (input.Current.MoveInput != Vector3.zero)
        {
            ...
            {
                moveDirection = Vector3.MoveTowards(moveDirection, LocalMovement() * backwardSpeed, WalkAcceleration * Time.deltaTime); 
            }
            //moveDirection = Vector3.MoveTowards(moveDirection, LocalMovement() * WalkSpeed, WalkAcceleration * Time.deltaTime);
        }
        else
